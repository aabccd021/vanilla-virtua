...
var createVirtualStore = (elementsCount, itemSize = 40, overscan = 4, ssrCount = 0, cacheSnapshot, shouldAutoEstimateItemSize = false) => {
  let isSSR = !!ssrCount;
  let stateVersion = [];
  let viewportSize = 0;
    ...
    $update: (type, payload) => {
      ...
      switch (type) {
        case ACTION_SCROLL: {
          ...
          if (!isJustJumped && // Ignore until manual scrolling
        ...
        case ACTION_ITEM_RESIZE: {
          ...
          applyJump(
            updated.reduce((acc, [index, size]) => {
              if (
                ...
                _scrollMode === SCROLL_BY_SHIFT || (_frozenRange ? (
                  ...
                  (_scrollDirection === SCROLL_IDLE && _scrollMode === SCROLL_BY_NATIVE ? getItemSize2(index) : 0) < getRelativeScrollOffset()
                ))
              ) {
          ...
          if (shouldAutoEstimateItemSize && viewportSize && // If the total size is lower than the viewport, the item may be a empty state
...
var createScrollObserver = (store, viewport, isHorizontal, getScrollOffset, updateScrollOffset, getStartOffset) => {
  ...
  const onWheel = (e) => {
    if (wheeling || // Scroll start should be detected with scroll event
    ...
    if (
      ...
      150 > timeDelta && 50 < timeDelta && // Get delta before checking deltaMode for firefox behavior
