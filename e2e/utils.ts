...
export const expectInRange = (value: number, { max, min }: { min: number; max: number }) => {
...
export const getLastItem = (scrollable: Locator, offset: { x?: number; y?: number } = {}) => {
  return scrollable.evaluate((s, { x: offsetX = 2, y: offsetY = 2 }) => {
    const rect = s.getBoundingClientRect();
    const el = document.elementFromPoint(rect.left + offsetX, rect.bottom - offsetY)!;
    const elRect = el.getBoundingClientRect();
    return {
      text: el.textContent,
...
export const getWindowFirstItem = (page: Page, offset: { x?: number; y?: number } = {}) => {
...
export const getWindowLastItem = (page: Page, offset: { x?: number; y?: number } = {}) => {
  return page.evaluate(({ x: offsetX = 2, y: offsetY = 2 }) => {
  ...
  return scrollable.evaluate((e) => {
    return new Promise<void>((resolve) => {
      ...
      const onScroll = () => {
        ...
        timer = setTimeout(() => {
          if (e.scrollLeft - (e as HTMLElement).offsetWidth - flakyOffset <= -e.scrollWidth) {
            e.removeEventListener("scroll", onScroll);
            resolve();
          } else {
            onScroll();
...
export const scrollWithTouch = (
  ...
  return scrollable.evaluate(async (e, { fromX, toX, fromY, toY, momentumScroll: isMomentumScrolling = false }) => {
    ...
    const createTouchEvent = (name: "touchstart" | "touchmove" | "touchend"): TouchEvent => {
    ...
    const touchMove = () => {
      setTimeout(() => {
        ...
        if (count > maxCount) {
          ...
          if (diffY) {
            e.scrollTop += diffY / maxCount;
          }
          if (diffX) {
            e.scrollLeft += diffX / maxCount;
          }
        }
...
export const listenScrollCount = (component: Locator, timeout = 2000): Promise<number> => {
