...
test.describe("check if it works when children change", () => {
	...
	test.skip("recovering when changed a lot after scrolling", async ({
		page,
	}) => {
		await page.goto(storyUrl("basics-vlist--increasing-items"));
		const component = await getScrollable(page);
...
test.describe("check if item shift compensation works", () => {
	...
	test.skip("keep start at mid when add to/remove from start", async ({
		page,
	}) => {
		const component = await getScrollable(page);
	...
	test.skip("prepending when total height is lower than viewport height", async ({
		page,
		browserName,
	}) => {
		const [component, container] = await Promise.all([
	...
	test.skip("stick to bottom even if many items are removed from top", async ({
		page,
		browserName,
	}) => {
		await page.goto(storyUrl("basics-vlist--increasing-items"));
	...
	test.skip("check if prepending cancels imperative scroll", async ({
		page,
	}) => {
		await page.goto(storyUrl("advanced-chat--default"));
		const component = await getScrollable(page);
...
test.describe("SSR and hydration", () => {
	...
	test.skip("check if smooth scrolling works after hydration", async ({
		page,
	}) => {
