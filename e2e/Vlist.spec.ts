...
test.describe("smoke", () => {
  ...
  test.skip("new window", async ({ page, context }) => {
...
test.describe("check if scroll jump compensation works", () => {
  ...
  test.skip("horizontal end -> start", async ({ page }) => {
    await page.goto(storyUrl("basics-vlist--horizontal"));
    const component = await getScrollable(page);
  ...
  test.skip("resize when its top is out of viewport", async ({ page }) => {
    ...
    expect(await container.evaluate((e) => e.children.length)).toBeGreaterThanOrEqual(3);
    ...
    const getTargetItem = () => {
      return container.evaluateHandle((e, i) => {
        return Array.from(e.children).find((c) => c.textContent?.startsWith(`${String(i)}Resize`))!;
      }, targetIndex);
    };
  ...
  test.skip("resize at bottom", async ({ page, browserName }) => {
    ...
    for (let i = 0; i <= 1; i++) {
      ...
      if (browserName === "firefox" ? Math.abs(bottom - prevBottom) <= 2 : bottom === prevBottom) {
        // succeeded
        return;
      }
    }
  ...
  test.skip("resize with smooth scroll", async ({ page }) => {
    await page.goto(storyUrl("advanced-collapse--collapse-and-scroll"));
    const [component, container] = await Promise.all([getScrollable(page), getVirtualizer(page)]);
  ...
  test.skip("stick to bottom", async ({ page }) => {
  ...
  test.skip("dynamic image", async ({ page, browserName }) => {
...
test.describe("check if item shift compensation works", () => {
  ...
  test.skip("check if prepending cancels imperative scroll", async ({ page }) => {
    await page.goto(storyUrl("advanced-chat--default"));
    const component = await getScrollable(page);
    // check if end is displayed
    const initialItem = await getLastItem(component);
...
test.describe("SSR and hydration", () => {
  test.skip("check if hydration works", async ({ page }) => {
    ...
    expect(await items.first().evaluate((e) => e.style.position)).not.toBe("absolute");
    ...
    expect(await items.first().evaluate((e) => e.style.position)).toBe("absolute");

    // should change state with scroll after hydration
    await component.evaluate((e) => e.scrollTo({ top: 1000 }));
    await page.waitForTimeout(500);
    expect(await component.locator(itemsSelector).count()).not.toBe(initialLength);
  });

  test.skip("check if smooth scrolling works after hydration", async ({ page }) => {
...
test.describe("emulated iOS WebKit", () => {
  test.describe("check if scroll jump compensation works", () => {
    test("scroll with touch", async ({ page }) => {
      ...
      const [w, h] = await page.evaluate(() => [window.outerWidth, window.outerHeight]);
      const centerX = w / 2;
      const centerY = h / 2;
      ...
      for (let i = 0; i < 5; i++) {
        ...
        await page.waitForTimeout(500);
        const [nextTop, nextLastItem] = await Promise.all([getScrollTop(component), getFirstItem(component)]);
